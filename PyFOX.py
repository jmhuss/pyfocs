...
for exp_name in experiment_names:
    ...
    if archive_read_flag:
        ...
        pyfocs.archive_read(internal_config[exp_name], write_mode=write_mode)
...
for exp_name in experiment_names:
    ...
    if calibrate_flag:
        ...
        for nraw, raw_nc in enumerate(ncfiles):
            ...
            dstemp, probe_names = pyfocs.assign_ref_data(dstemp, cal, ref_data=ref_data)
            ...
            if not cal["double_ended"]:
                ...
                if step_loss_flag:
                    ...
                    for spl_num, spl_LAF in enumerate(splice_LAF):
                        dstemp["logPsPas"] = dstemp.logPsPas.where(
                            (dstemp.LAF < spl_LAF),
                ...
                if cal["method"] == "matrix":
                    ...
                    dstemp = pyfocs.labelLoc_additional(
                        dstemp, cal["library"], "calibration"
                    )
                    cal_baths = [
                        c
                        for c in cal["library"]
                ...
                if cal["method"] == "ols single" or cal["method"] == "wls single":
                    dstore = pyfocs.data.to_datastore(dstemp, internal_config, False)
                    method = cal["method"].split()[0]
                    dstore = pyfocs.single_calibrate(dstore, method=method)
                    dstemp = pyfocs.data.from_datastore(dstore, datavars=probe_names)
                ...
                bw_raw_nc = raw_nc.replace(fw_channel, bw_channel)
                bw_dstemp = xr.open_dataset(bw_raw_nc)
                bw_dstemp, _ = pyfocs.assign_ref_data(bw_dstemp, cal, ref_data=ref_data)
                ...
                if not len_fw == len_bw:
                    ...
                    if abs(len_time_off) > 2:
                        ...
                        mess = (
                            "The time dimension between the forward and "
                    ...
                    if len_time_off < 0 or (nraw + 1) == ntot:
                        bw_dstemp = bw_dstemp.reindex_like(dstemp, method="nearest")

                    # The forward channel has more time steps.
                    elif len_time_off > 0 and not (nraw + 1) == ntot:
                        file_list = ncfiles[nraw : nraw + 1]
                        file_list = [
                            fl.replace(fw_channel, bw_channel) for fl in file_list
                        ]
                        bw_dstemp = xr.open_mfdataset(file_list, combine="by_coords")
                        bw_dstemp.load()
                        bw_dstemp["time"] = bw_dstemp["time"] - pd.Timedelta(dt)
                        bw_dstemp = bw_dstemp.reindex_like(dstemp, method="nearest")
                        bw_dstemp, _ = pyfocs.assign_ref_data(
                            bw_dstemp, cal, ref_data=ref_data
                        )

                bw_dstemp = pyfocs.to_datastore(bw_dstemp, internal_config, True)
                dstemp = pyfocs.to_datastore(dstemp, internal_config, True)
                dstemp = pyfocs.merge_single(
                    dstemp, bw_dstemp, fixed_shift=cal["fixed_shift"]
                )
                ...
                for probe in sections:
                    ...
                    # Delete from the last index to the beginning so that the index
                    # always works.
                    delete_index.sort()
            ...
            dstemp = pyfocs.labelLoc_additional(dstemp, cal["library"], "calibration")
            if "phys_locs" in internal_config:
                for ploc in internal_config["phys_locs"]:
                    # Label all physical locations provided
                    dstemp = pyfocs.labelLoc_additional(dstemp, lib[ploc], ploc)
...
if final_flag:
    print("-------------")
    print(
        "Final preparation: assigning physical coordinates, dropping unused fields and locations"
    )
    print(" ")
    ...
    for exp_name in experiment_names:
        ...
        if cal_suffix:
            ncfiles = [
                file
                for file in contents
                if ".nc" in file and "cal" in file and cal_suffix in file
            ]
        else:
        ...
        for ncal, cal_nc in enumerate(ncfiles):
            ...
            if not dt_start == dt_end:
                ...
                dstemp = dstemp.interp(
                    time=reg_time_pd, kwargs={"fill_value": "extrapolate"}
                )

            # Clean up unused variables and labels.
            vars_to_drop = [v for v in dstemp.data_vars if v not in vars_to_keep]
            coords_to_drop = [c for c in dstemp.coords if c not in coords_to_keep]
                ...
                for map_from, map_to in locs_to_match.items():
                    ...
                    for section, shift in shifted_sections[map_from].items():
                        ...
                        ds_map_from = pyfocs.labeler.dtsPhysicalCoords_3d(
                            ds_map_from, temp_lib[map_from], reset_midx=False
                        )
                    ...
                    ds_ploc_map_from = pyfocs.labelLoc_additional(
                        dstemp, temp_lib[map_from], map_from
                    )

                    for uns in unshifted_sections[map_from]:
                        dstemp_uns = ds_ploc_map_from.where(
                            ds_ploc_map_from[map_from] == uns, drop=True
                        )
                        ...
                        dstemp_uns = pyfocs.labeler.dtsPhysicalCoords_3d(
                            dstemp_uns, temp_lib[map_from], reset_midx=False
                        )
            ...
            for ploc in phys_locs:
                ...
                dstemp_ploc = pyfocs.labelLoc_additional(dstemp, lib[ploc], ploc)
                ...
                outname = (
                    "_".join(
                        filter(
                            None, [exp_name, "final", outname_date, final_suffix, ploc]
                        )
                    )
                    + ".nc"
                )
