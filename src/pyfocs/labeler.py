...
# ------------------------------------------------------------------------------
def dtsPhysicalCoords(
    ds, location, loc_field="loc_general", coord_opt="relative", align="right"
):
    """
...
def dtsPhysicalCoords_3d(
    ...
    for l in location:
        ...
        if reverse:
            ...
            section[datavar] = (
                ("time", "LAF"),
                np.flip(section[datavar].values, axis=1),
            )
        ...
        if (
            "distance_check" in location[l] and location[l]["distance_check"]
        ...
        midx = pd.MultiIndex.from_arrays([x_int, y_int, z_int], names=("x", "y", "z"))
