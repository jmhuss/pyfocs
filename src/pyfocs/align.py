...
def section_shift_x(
    ...
    if fixed_shift is None:
        ...
        c = pyfocs.stats.norm_xcorr(
            s1_vals, s2_vals, scaleopt="coef", remove_mean=True, lag=lag
        )
    ...
    if plot_results:
        ...
        ax.text(
            0.15,
            0.85,
            "max correlation at shift " + str(shift_x),
        ...
        ax.plot(s1.x, s1[temp_field].mean(dim="time"), marker=".", label=ploc1)
        ax.plot(
            s2_shift.x, s2_shift[temp_field].mean(dim="time"), marker=".", label=ploc2
        )
        ax.set_xlabel("Relative LAF [m]")
        ax.set_ylabel("Temperature [C]")
        ax.legend(loc="upper right")
...
def interp_section(
    ...
    s1, s2, shift_x, c, lags = section_shift_x(
        ds,
        fn_lib,
        ploc1,
        ploc2,
    ...
    sub_s2.attrs["reverse"] = s1_reverse
    ...
    if plot_results:
        ...
        if fixed_shift is None:
            ...
            ax.text(
                0.15,
                0.85,
                "max correlation at shift " + str(shift_x) + "m",
                transform=ax.transAxes,
            )
            ax.set_title(
                "a) Cross-correlation between " + ploc1 + " and " + ploc2, loc="left"
            )
        ...
        ax.plot(
            sub_s2_mean.x,
            sub_s2_mean[temp_field].T,
            marker="o",
            label=ploc2 + " (interpolated)",
            color=colr2_interp,
        )

        ax.plot(
            [np.min(sub_s1.x.values), np.min(sub_s1.x.values)],
            [
        ...
        ax.plot(
            [np.max(sub_s1.x.values), np.max(sub_s1.x.values)],
            [
                s1[temp_field].min().values - delta_T,
                s1[temp_field].max().values + delta_T,
            ],
        ...
        ax.plot(
            sub_s1_mean.differentiate(coord="x").x,
        ...
        ax.plot(
            sub_s2_mean.differentiate(coord="x").x,
        ...
        ax.plot([np.min(sub_s1.x.values), np.min(sub_s1.x.values)], [-1.5, 1.5], "k-")
        ax.plot([np.max(sub_s1.x.values), np.max(sub_s1.x.values)], [-1.5, 1.5], "k-")
...
def section_limits(
    s1, s2, fn_lib, ploc1, ploc2, label, plot_results=False, temp_field="cal_temp"
):
    """
    ...
    if s2_reverse:
        ...
        sub_s2 = s2.swap_dims({"LAF": "x"}).sel(
            x=slice(np.max(sub_s1.x.values), np.min(sub_s1.x.values))
        )
    else:
        loc_s2 = [np.min(sub_s1.x.values), np.max(sub_s1.x.values)]
        sub_s2 = s2.swap_dims({"LAF": "x"}).sel(
            x=slice(np.min(sub_s1.x.values), np.max(sub_s1.x.values))
        )

    # Return the adjusted LAF values based on aligning s2 to s1
    fn_lib[ploc2][label]["LAF"] = [
        float(sub_s2.LAF.isel(x=0).values),
