...
def matrixInversion(dsCal, cfg):
    ...
    gamma = np.ones(np.shape(dsCal.time.values)) * -9999.0
    C = np.ones(np.shape(dsCal.time.values)) * -9999.0
    delta_alpha = np.ones(np.shape(dsCal.time.values)) * -9999.0
    manualTemp = np.ones(np.shape(dsCal.temp.values)) * -9999.0

    # Within each time step solve for the calibration parameters
    for n, t in enumerate(dsCal.time):
        sys.stdout.write(
            "\r" + "Time step " + str(n + 1) + " of " + str(np.size(dsCal.time.values))
        )
        ...
        manualTemp[n] = gamma[n] / (
            dsCal.logPsPas.sel(time=t) + C[n] - delta_alpha[n] * dsCal.LAF
        )
