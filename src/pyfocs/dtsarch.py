...
def round_dt(dt, delta, type="floor"):
    ...
    if type == "floor":
        dt = dt - datetime.timedelta(
            minutes=dt.minute % delta, seconds=dt.second, microseconds=dt.microsecond
        )
    elif type == "ceil":
        dt = dt + datetime.timedelta(
            minutes=delta - dt.minute % delta - 1,
            seconds=60 - dt.second - 1,
            microseconds=10**6 - dt.microsecond,
        )
    else:
        raise ValueError(
            "Unrecognized dt rounding type. Valid options are" "floor and ceil."
        )
    return dt
    ...
    for ch in channels:
        ...
        while xml_counts <= len(contents):
            ...
            if xml_counts == len(contents):
                if mode == "archiving":
                    # Create file names for this hour
                    year, month, day, hour, minute, _, _ = dt_strip(
                        dt1, str_convert=True
                    )
