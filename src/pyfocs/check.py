...
def config(fn_cfg, ignore_flags=False):
        ...
        if dir_ext is not None:
            ...
            if not os.path.exists(dir_ext):
                raise FileNotFoundError(
                    "External data directory not found at\n" + dir_ext
                )
    except KeyError:
        dir_ext = None
    ...
    if ignore_flags:
        ...
        for cloc in cal["library"]:
            ...
            assert len(calsect["LAF"]) == 2, missing_mess.format(cl=cloc, lib=calsect)
    ...
    # - comprehensive calibration check
        ...
        if "external_fields" in cal and cal["external_fields"]:
            ...
            if dir_ext:
                ext_fname = os.path.join(
                    dir_ext, cfg["directories"]["filename_external"]
                )
                if os.path.isfile(ext_fname):
                    # Make sure the file is a netcdf
                    if not ext_fname.endswith(".nc"):
                        mess = (
                            "Config file indicates to use external data, "
                    ...
                    mess = (
                        "Config file indicates to use external data, but no file "
        ...
        if cal["builtin_probe_names"]["probe2Temperature"]:
            ...
            cal["builtin_flag"] = True
        if (
            not cal["builtin_probe_names"]["probe1Temperature"]
            and not cal["builtin_probe_names"]["probe2Temperature"]
        ):
        ...
        double_methods = ["ols double", "wls double", "temp_matching"]
        ...
        if cal["method"] not in valid_meths:
            mess = (
                "The calibration method, {cm}, is not in the list of "
        ...
        if cal["method"] in single_methods:
            ...
            if not dt:
                mess = (
                    "A resampling time must be provided when performing "
        ...
        if cal["method"] == "matrix":
            cal_baths = [
                c for c in cal["library"] if cal["library"][c]["type"] == "calibration"
            ]
            if not len(cal_baths) == 3:
                mess = (
                    "The explicit matrix inversion for the single-ended "
        ...
        for cloc in cal["library"]:
            ...
            if refsen not in probe_names:
                raise KeyError(
                    miss_ref_sen_mess.format(r=refsen, cl=cloc, slist=probe_names)
                )
            # Bath types must be valid
            if bath_type not in valid_bath_types:
                raise KeyError(
                    unkn_cal_mess.format(ct=bath_type, cl=cloc, valopt=valid_bath_types)
                )
    ...
    for exp_name in experiment_names:
        ...
        if "_" in exp_name:
            mess = (
                "Experiment names cannot contain underscores. "
        ...
        if "raw_xml" in remote_dir:
            dir_raw_xml = os.path.join(
                dir_pre_remote, exp_name, cfg["directories"]["raw"]
            )
        else:
            dir_raw_xml = os.path.join(
                dir_pre_local, exp_name, cfg["directories"]["raw_xml"]
            )
        ...
        if (not os.path.exists(dir_raw_xml)) and (cfg["flags"]["archiving_flag"]):
            mess = (
                "Creating archived data is on but the raw xml data "
        ...
        if "archive" in remote_dir:
            dir_archive = os.path.join(
                dir_pre_remote, exp_name, cfg["directories"]["archive"]
            )
        else:
            dir_archive = os.path.join(
                dir_pre_local, exp_name, cfg["directories"]["archive"]
            )
        ...
        if not os.path.exists(dir_archive):
            ...
            if cfg["flags"]["archiving_flag"]:
                ...
                print(
                    "Archived directory not found. Created a new one at " + dir_archive
                )
            # throw error if archived files are needed but neither found nor created
            elif cfg["flags"]["archive_read_flag"]:
                mess = (
                    "Reading archived data is on but the archived data "
        ...
        if "raw_netcdf" in remote_dir:
            dir_raw_netcdf = os.path.join(
                dir_pre_remote, exp_name, cfg["directories"]["raw_netcdf"]
            )
        else:
            dir_raw_netcdf = os.path.join(
                dir_pre_local, exp_name, cfg["directories"]["raw_netcdf"]
            )
        ...
        if not os.path.exists(dir_raw_netcdf):
            ...
            if cfg["flags"]["archive_read_flag"]:
                ...
                print(
                    "Calibrated directory not found. Created a new one at "
                    + dir_raw_netcdf
                )
            ...
            # nor created
                ...
                mess = (
                    "Calibration is on but the raw netcdf data "
        ...
        if "calibrated" in remote_dir:
            dir_cal = os.path.join(
                dir_pre_remote, exp_name, cfg["directories"]["calibrated"]
            )
        else:
            dir_cal = os.path.join(
                dir_pre_local, exp_name, cfg["directories"]["calibrated"]
            )
        ...
        if not os.path.exists(dir_cal):
            ...
            if cfg["flags"]["calibrate_flag"]:
                os.makedirs(dir_cal)
                print("Calibrated directory not found. Created a new one at " + dir_cal)
            elif cfg["flags"]["final_flag"]:
                mess = (
                    "Finalizing is on but the calibrated data "
        ...
        if "final" in remote_dir:
            dir_final = os.path.join(
                dir_pre_remote, exp_name, cfg["directories"]["final"]
            )
        else:
            dir_final = os.path.join(
                dir_pre_local, exp_name, cfg["directories"]["final"]
            )

        # Create the folder for the final files if it doesn't already exist
        if (not os.path.exists(dir_final)) and (
            (cfg["flags"]["calibrate_flag"]) or (cfg["flags"]["final_flag"])
        ):
        ...
        if "graphics" in remote_dir:
            dir_graphics = os.path.join(
                dir_pre_remote, exp_name, cfg["directories"]["graphics"]
            )
        else:
            dir_graphics = os.path.join(
                dir_pre_local, exp_name, cfg["directories"]["graphics"]
            )
    ...
    if not ignore_flags and (in_cfg["flags"]["final_flag"] or label_data_flag):
        ...
        try:
            ...
            mess = (
                "The phys_locs field in dataProperties is empty but "
        ...
        if "calibration" in phys_locs:
            mess = (
                "Including the calibration locations in the location "
    ...
    missing_mess = (
        "Coordinates for {ploc} were not defined by pairs of LAF, "
        ...
        for loc_type_cur in phys_locs:
            ...
            for l in cfg["location_library"]:
                if loc_type_cur == cfg["location_library"][l]["loc_type"]:
                    ...
                    assert len(lib[loc_type_cur][l]["LAF"]) == 2, missing_mess.format(
                        ploc=l, lib=lib[loc_type_cur][l]
                    )
                    if any(np.isnan(lib[loc_type_cur][l]["LAF"])):
                        del lib[loc_type_cur][l]
                        print(
                            l
                            + " will not be labeled due to NaNs in LAF. Check library."
                        )
                        continue

                    # All physical coordinates are defined by pairs of x, y, z
                    if cfg["flags"]["final_flag"] and loc_type_cur in phys_locs:
                        assert "x_coord" in lib[loc_type_cur][l], missing_mess.format(
                            ploc=l, lib=lib[loc_type_cur][l]
                        )
                        assert "y_coord" in lib[loc_type_cur][l], missing_mess.format(
                            ploc=l, lib=lib[loc_type_cur][l]
                        )
                        assert "z_coord" in lib[loc_type_cur][l], missing_mess.format(
                            ploc=l, lib=lib[loc_type_cur][l]
                        )

                        assert (
                            len(lib[loc_type_cur][l]["x_coord"]) == 2
                        ), missing_mess.format(ploc=l, lib=lib[loc_type_cur][l])
                        assert (
                            len(lib[loc_type_cur][l]["y_coord"]) == 2
                        ), missing_mess.format(ploc=l, lib=lib[loc_type_cur][l])
                        assert (
                            len(lib[loc_type_cur][l]["z_coord"]) == 2
                        ), missing_mess.format(ploc=l, lib=lib[loc_type_cur][l])
    ...
    if check_loc_library and "location_matching" in cfg["dataProperties"] and stacked:
        ...
        for map_from in cfg["dataProperties"]["location_matching"]:
            ...
            if map_to not in lib or map_from not in lib:
                print(
                    "Location types to be matched were not found in the location library."
                )
                raise KeyError
            ...
            if not common_sections:
                print(
                    map_to + " and " + map_from + " do not have common section names."
                )
                raise KeyError

            # Verify that the fixed_shift values are a number
            f_shift = cfg["dataProperties"]["location_matching"][map_from][
                "fixed_shift"
            ]
            assert all(
                [isinstance(s, (float, int)) for f, s in f_shift.items()]
            ), "Fixed shifts must be a number."

            # Verify that the fixed_shift sections exist in the location library.
            mess = (
                "Not all fixed shift sections could be found in the "
                "location library for {mf}"
            ).format(mf=map_from)
            assert all([f in lib[map_from] for f in f_shift.keys()]), mess
            mess = (
                "Not all fixed shift sections could be found in the "
                "location library for {mt}"
            ).format(mt=map_to)
            assert all([f in lib[map_to] for f in f_shift.keys()]), mess

            # Determine which sections are not going to be shifted.
            unshifted_map_from = list(
                set(lib[map_from]).difference(set(f_shift.keys()))
            )
            ...
            if set(common_sections).symmetric_difference(set(f_shift.keys())):
                mess = (
                    "These sections are common to {map_from} and {map_to} "
        ...
        in_cfg["phys_locs"] = list(
            set(lib.keys()).difference(set(in_cfg["location_matching"].keys()))
        )

    # Require the location library to be in stacked format.
    elif (
        check_loc_library
        and "location_matching" in cfg["dataProperties"]
        and not stacked
    ):
        mess = (
            "When including the location_mapping option, the location "
    ...
    if (
        "step_loss_LAF" in cfg["dataProperties"]
        and "step_loss_correction" in cfg["dataProperties"]
    ):
        in_cfg["step_loss"] = {}
        steploss_laf = np.atleast_1d(cfg["dataProperties"]["step_loss_LAF"]).astype(
            "float"
        )
        steploss_corr = np.atleast_1d(
            cfg["dataProperties"]["step_loss_correction"]
        ).astype("float")
    ...
    if "fiber_limits" in cfg["dataProperties"]:
        if cfg["dataProperties"]["fiber_limits"]["min_limit"]:
            in_cfg["min_fiber_limit"] = cfg["dataProperties"]["fiber_limits"][
                "min_limit"
            ]
        else:
        ...
        if cfg["dataProperties"]["fiber_limits"]["max_limit"]:
            in_cfg["max_fiber_limit"] = cfg["dataProperties"]["fiber_limits"][
                "max_limit"
            ]
        else:
    ...
    if "cal_suffix" in cfg["directories"]:
        ...
        if in_cfg["cal_suffix"] and "_" in in_cfg["cal_suffix"]:
            mess = (
                "File suffixes cannot contain underscores. "
                "This character is reserved by the pyfocs naming scheme."
            )
            raise ValueError(mess)
    else:
    ...
    if "fiber_limits" in cfg["dataProperties"] and (
        not in_cfg["outname_suffix"]
        or not in_cfg["cal_suffix"]
        or not in_cfg["final_suffix"]
    ):
        warn = (
            "Fiber limits were provided without a suffix. This may cause "
