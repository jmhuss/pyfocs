...
def to_datastore(ds, config, double):
    ...
    if not [ref for pr in sections for ref in sections[pr]]:
        mess = (
            "No reference sections of type calibration were found. "
...
def from_datastore(dstore, datavars=None, coords=None, return_cal_params=False):
    """
    Convert the pyfocs version of an xarray dataset into a dtscalibration
    datastore object.
...
def double_end_dv_clean(ds):
    ...
    drop_vars = [
        dv for dv in ds.data_vars if dv not in acc_datavars and "x" in ds[dv].coords
    ]
...
def merge_single(
    ...
    double = merge_double_ended(
        ds_fw=dstore_fw, ds_bw=dstore_bw, cable_length=cable_length, plot_result=False
    )
    ...
    if not fixed_shift:
        ...
        # Otherwise use the smaller shift the two lengths should be close to each other.
            ...
            mess = (
                "When merging the single-ended observations into "
...
def single_calibrate(single, method):
    ...
    if method == "ols":
        ...
        # And calibrate
        single.calibration_single_ended(st_var=st_var, ast_var=ast_var, method=method)
...
def assign_ref_data(dstemp, cal, ref_data=None):
    ...
    if cal["external_flag"]:
        temp_ref_data = ref_data.reindex_like(dstemp.time, method="nearest")
